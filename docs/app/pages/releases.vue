...
const { data: releases } = await useFetch<GitHubRelease[]>("https://api.github.com/repos/genu/nuxt-upload-kit/releases", {
  headers: {
...
<template>
  <UPage>
    <UPageHero title="Releases" description="All notable changes to Nuxt Upload Kit." :ui="{ container: 'lg:py-16' }" />
    ...
    <UPageBody>
      <UContainer :ui="{ base: 'max-w-3xl' }">
        ...
        <UEmpty v-else icon="i-lucide-git-branch" title="No releases yet" description="Check back soon for the first release!" />
      </UContainer>
    </UPageBody>
  </UPage>
</template>
