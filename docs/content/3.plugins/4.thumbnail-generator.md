...
<template>
  <div v-for="file in uploader.files" :key="file.id" class="file-preview">
    ...
    <!-- PDF icon -->
    <div v-else-if="file.mimeType === 'application/pdf'" class="icon">📄</div>

    <!-- Document icon -->
    <div v-else-if="file.mimeType.includes('document')" class="icon">📝</div>
...
Thumbnail upload failures are **non-fatal** — the main file upload completes successfully even if the thumbnail upload fails.
