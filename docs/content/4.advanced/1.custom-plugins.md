...
description: Create your own validators, processors, and storage plugins.
---

# Custom Plugins

Extend Nuxt Upload Kit with custom plugins for validation, processing, and storage.

## Plugin Types

There are two plugin types you can create:

1. **Processing Plugins** - For validation and file transformation
2. **Storage Plugins** - For uploading to custom storage providers

## Processing Plugins

Use `defineProcessingPlugin` to create validators and processors:
...
Use `defineStoragePlugin` to create custom storage providers:

```ts
import { defineStoragePlugin } from "nuxt-upload-kit"
...
export const PluginMyStorage = defineStoragePlugin<MyStorageOptions, MyStorageResult>((options) => ({
  id: "my-storage",
  hooks: {
...
| Hook            | When             | Purpose              | Return             |
| --------------- | ---------------- | -------------------- | ------------------ |
| `upload`        | During upload    | Send file to storage | `{ url, ...data }` |
| `getRemoteFile` | Loading existing | Fetch file metadata  | File metadata      |
| `remove`        | Deleting file    | Delete from storage  | `void`             |
