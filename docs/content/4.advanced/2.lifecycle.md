...
│  • file.status = 'waiting'                                      │
│                                                                 │
│  If autoProceed: true → Continue to upload                      │
...
await uploader.initializeExistingFiles([
  ...
  { id: 'path/to/file2.png', preview: 'custom-preview-url' }
])
```
...
if (import.meta.dev) {
  const events = [
    'file:added', 'file:removed', 'file:replaced', 'file:error',
    'upload:start', 'upload:progress', 'upload:complete',
    'image-compressor:start', 'image-compressor:complete'
  ]
