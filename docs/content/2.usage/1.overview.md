...
Storage adapter is registered in `server/upload.server.config.ts` and restrictions live in `nuxt.config.ts > uploadKit.restrictions` — `useUploadKit()` only configures component-level options like processors and auto-upload.
