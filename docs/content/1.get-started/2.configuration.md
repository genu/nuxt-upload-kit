...
::prose-tip
The storage adapter and global `restrictions` live in `nuxt.config.ts` (and `server/upload.server.config.ts`). `useUploadKit({ restrictions })` is only used when a specific component needs different limits than the app-wide defaults.
::
