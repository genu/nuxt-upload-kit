...
Nuxt Upload Kit can be configured at the module level in `nuxt.config.ts` or per-instance when using `useUploadManager`.
...
export default defineNuxtConfig({
  ...
  uploadKit: {
    // Auto-import the useUploadManager composable (default: true)
    autoImport: true
  }
})
```
...
|--------|------|---------|-------------|
| `autoImport` | `boolean` | `true` | Auto-import the `useUploadManager` composable |

## Upload Manager Options

The `useUploadManager` composable accepts a configuration object:
