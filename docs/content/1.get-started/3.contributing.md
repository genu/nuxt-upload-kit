...
│       ├── shared/                  # Isomorphic rule core (restrictions, rules, errors)
│       ├── server/
│       │   ├── handlers/            # /presign, /direct, /download, /delete
│       │   ├── adapters/            # S3, Azure, Firebase storage adapters
│       │   ├── plugins/bootstrap.ts # Resolves mode at server startup
│       │   ├── capabilities.ts      # Adapter capability derivation
│       │   ├── restrictions.ts      # Server-side restriction enforcement
│       │   └── utils.ts             # Handler helpers (file ID, decoding, etc.)
│       └── composables/
│           ├── useUploadKit/        # Main composable
│           │   ├── index.ts         # Core upload manager
│           │   ├── types.ts         # Type definitions
│           │   └── plugins/         # Processing plugins (compressors, thumbnails)
│           │       └── storage/     # Built-in transports + escape-hatch adapters
...
### Custom Validator Plugin Example

For static rules (size, MIME type, count), extend `Restrictions` in `src/runtime/shared/restrictions.ts` — both runtimes pick it up automatically. Use a processing plugin only for client-side rules that can't be expressed declaratively (async checks, stateful logic, content inspection):
